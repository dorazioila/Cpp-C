...
void AfficheEnteteVehiculeHV ()
{
 ...
 sprintf(Tampon,"%s","Couleur") ; MonPrintf(Tampon,10,strlen(Tampon)) ;
 printf("\n") ;
}
...
void AfficheVehiculeHV (struct VehiculeHV    *UnRecord)
{
 ...
 sprintf(Tampon,"%s",UnRecord->Couleur ) ; MonPrintf(Tampon,10,strlen(Tampon)) ;
 printf("\n") ;
}
   ...
   case '4': 
       {
        ...
        printf("Donner la reference du vehicule a rechercher : ");
        if (fgets(tampon, sizeof(tampon), stdin) == NULL)
            break;
        ref = atoi(tampon);

        int resultat = RechercheHV("VehiculesHV", ref, &Record);

        if (resultat == 1)
        {
            printf("\nVehicule trouve :\n", ref);
            AfficheEnteteVehiculeHV;
            AfficheVehiculeHV(&Record);
        }
        else if(resultat==0)
        {
            printf("\nAucun vehicule trouve avec la reference %d \n", ref);
        }
        else
        {
    ...
    case '5': 
{
    ...
    printf("Reference du vehicule souhaite : ");
    fgets(tampon, sizeof(tampon), stdin);
    ...
    // 1️ Réserver le véhicule (décrémenter le stock)
    res = ReservationHV("VehiculesHV", Reference, Quantite);
    if (res == -1)
    {
        printf("Erreur ouverture fichier VehiculesHV\n");
    }
    else if (res == 0)
    {
        printf("Stock insuffisant ou reference incorrecte\n");
    }
    else 
    {
        printf("Reservation OK, stock mis a jour.\n");

        // Générer la facture
        res = FacturationHV("FactureHV", NomClient, maintenant, Quantite, Reference);
        if (res == -1)
            printf("Erreur creation facture\n");
        else
            printf("Facture enregistree avec succes\n");
    }
