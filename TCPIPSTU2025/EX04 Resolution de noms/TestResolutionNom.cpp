...
printf("Pour inet 192.168.17.128 : 1300  :\n");
if (getaddrinfo("192.168.17.128","1300",&hints,&results) != 0)
printf("Erreur de getaddrinfo");
else
{
...
{
struct addrinfo* addr;
  int result = getaddrinfo("orbazio", NULL, NULL, &addr);
  if (result != 0) {
    printf("Error from getaddrinfo: %s\n", gai_strerror(result));
  }
  else {
  struct sockaddr_in* internet_addr = (struct sockaddr_in*) addr->ai_addr;
  printf("orbazio is at: %s\n", inet_ntoa(internet_addr->sin_addr));
}
}
