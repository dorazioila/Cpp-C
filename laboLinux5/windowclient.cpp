...
void WindowClient::on_pushButtonEnvoyer_clicked()
{
  ...
  //recup du texte a envoyer
  const char* txt = getAEnvoyer();
  if(txt == nullptr){
    fprintf(stderr,"aucun texte a envoyer \n");
    return ;
  }
  //prepa du message
  m.type = 1;
  strncpy(m.texte, txt, sizeof(m.texte));
  

  //envoie du message
  if(msgsnd(idQ, &m, sizeof(m.texte),0)==-1){
    perror("msgsnd");
    return;
  }
  fprintf(stderr,"envoie du message reussi \n");

  //reception du message
  if(msgrcv(idQ, &recu, sizeof(recu.texte),1,0) == -1){
    perror("msgrcv");
    return ;
  }
  fprintf(stderr,"message bien recu \n");
