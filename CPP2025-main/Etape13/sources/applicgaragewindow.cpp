...
void ApplicGarageWindow::on_pushButtonNewContract_clicked()
{
    ...
    {
        dialogError("Erreur", "Aucun vendeur connecté");
        return;
    }

    int indexClient = getIndexClientSelectionTable();
    if (indexClient == -1)
    {
        dialogError("Erreur", "Aucun client sélectionné");
        return;
    ...
    int vendeurId = Garage::idLoggedEmployee;
    int clientId = g.findClientByIndex(indexClient).getId();

    string projectName = getCurrentProjectName();
    if (projectName == "" || projectName == "---")
    {
        dialogError("Erreur", "Aucun projet de voiture");
        return;
    }
    
    Car& car = Garage::getCurrentProject();
    car.setName(projectName);
    car.save();              

    g.addContract(vendeurId, clientId, projectName);
...
void ApplicGarageWindow::on_pushButtonDeleteContract_clicked()
{
    ...
    if (index == -1)
    {
        dialogError("Erreur", "Aucun contrat sélectionné");
        return;
    }

    g.deleteContractByIndex(index);
...
void ApplicGarageWindow::on_pushButtonShowCar_clicked()
{
    ...
    Contract c = g.findContractByIndex(index);

    Car& car = Garage::getCurrentProject();
    car.load(c.getProjectName());
